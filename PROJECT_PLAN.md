...
- [x] Зафиксировать имя проекта, лицензию, репозиторий
- [x] Настроить `go.mod`, базовый CI (lint + test на пустом проекте)
- [x] Написать `ARCHITECTURE.md` и первый ADR (выбор Go+cgo+C++ вместо чистого Go)
- [x] `hostix doctor` — детект наличия Docker/Tart в системе (это нужно раньше всего остального)
